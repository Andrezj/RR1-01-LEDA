...
public class RepositorioProdutoArrayList {
	...
	private int procurarIndice(int codigo) {
		...
		int i = 0;
		while(contem == false && i <= this.produtos.size() - 1) {
			Produto comparado = (Produto) this.produtos.get(i);
			if (comparado.getCodigo() == codigo) {
				contem = true;
			}
			i ++;
		}
	...
	public void remover(int codigo) {
		Produto ARemover = this.procurar(codigo);
		this.produtos.remove(ARemover);
		index --;
	}
